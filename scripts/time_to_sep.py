...
def get_path_remote(root, obsname, version):
    """
    Get the path on nas given the obsname
    """
    date_obsid, orbit = obsname.rsplit("_", maxsplit=1)
    base_path = f"{root}/{date_obsid}/czti/orbit/{orbit}_V{version}.0"
...
if __name__ == "__main__":
    ...
    for ev in event_obs:
        ...
        #     pass
            ...
            # print(ev)
        # else:
        mkf, bc, lv = get_path_remote("/mnt/nas2_czti/czti/level2", ev, 1) #version is 1 right now
        print(mkf)
            # print(bc)
            # print(lv)
    # if args.neighbour:
...
trigtime = Time(args.triggertime)
...
transient_theta, transient_phi, transient_thetax, transient_thetay, coo_x, coo_y, coo_z, coo_transient, earth, earth_czti, earth_transient, earth_occult_angle = angles.txy(mkf, czti_time, ra_pnt.deg, dec_pnt.deg)

print("Earth Transient Angle: ",earth_transient)
